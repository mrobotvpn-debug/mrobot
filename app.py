...
def poll_main_bot():
    print("Запущен опрос основного бота...")
    while True:
        try:
            ...
            for u in r.get("result", []):
                ...
                # Определяем chat_id
                chat_id = None
                if msg: chat_id = msg.get("chat", {}).get("id")
                if cb: chat_id = cb.get("from", {}).get("id")
                ...
                if "successful_payment" in msg:
                    ...
                    kb = {"inline_keyboard": [[{"text":"ПК", "callback_data":f"plat_pc_{payload}"},{"text":"АПК", "callback_data":f"plat_apk_{payload}"}]]}
                    send(MAIN_BOT_TOKEN, chat_id, "Оплата принята. Выберите платформу:", kb)
                    continue
                ...
                if cb:
                    ...
                    if data == "get_test":
                        ...
                        send(KEYS_BOT_TOKEN, ADMIN_CHAT_ID, adm_msg, {"inline_keyboard": [[{"text":"Выдать ключ", "callback_data":f"auto_gen_{chat_id}_{p[3]}_{p[1]}"}]]})
                ...
                if text == "/start":
                    ...
                    send(MAIN_BOT_TOKEN, chat_id, "Добро пожаловать в Mrobot VPN! Купите ключ или возьмите тест:", kb)
            ...
            print(f"Ошибка в poll_main_bot: {e}")
            time.sleep(5)

def poll_keys_bot():
    print("Запущен опрос бота ключей...")
    while True:
        try:
